...
class Classifier(
    ...
    private val MAX_POSSIBLE_DETECTIONS = 10
    ...
    fun recognizeImage(bitmap: Bitmap): List<Recognition> {
        ...
        outputLocations = Array(
            1
        ) { Array(MAX_POSSIBLE_DETECTIONS) { FloatArray(4) } }
        outputClasses = Array(1) { FloatArray(MAX_POSSIBLE_DETECTIONS) }
        outputScores = Array(1) { FloatArray(MAX_POSSIBLE_DETECTIONS) }
        ...
        val numDetectionsOutput = Math.min(MAX_POSSIBLE_DETECTIONS, numDetections[0].toInt())
        val recognitions =
